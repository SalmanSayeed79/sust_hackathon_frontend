...
export default function ProductDetails() {
    ...
    const getData=async()=>{
        const res = await axios.get("http://localhost:8000/listing/"+params.id)
  ...
  return (
    ...
    <button className='fixed top-8 rounded-full p-3 bg-amber-500 left-8 z-20' onClick={()=>window.history.back()}>
        <MdArrowBackIos size={24} />
    </button>
    <button className='fixed top-8 z-20 rounded-full p-3 bg-amber-500 right-8'>
        <BiCart size={24} />
    </button>
